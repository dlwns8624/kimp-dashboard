...
export default function WhaleWatch() {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 overflow-hidden">
      ...
      {/* ── Filters ── */}
      <div className="flex flex-wrap gap-1.5 p-2.5 border-b border-neutral-800 shrink-0">
        ...
        {(["h1","h4","h12","h24"] as const).map((k, i) => {
          ...
          return (
            <div key={k} className={`p-2 ${i < 3 ? "border-r border-neutral-800" : ""}`}>
              <p className="text-[8px] font-black text-neutral-600 mb-1">
              ...
              <div className="mt-1.5 h-1 rounded-full overflow-hidden bg-neutral-800 flex">
      ...
      <div className="px-2 pt-2 pb-1 border-b border-neutral-800 shrink-0">
        <p className="text-[8px] font-bold text-neutral-600 mb-1">
          1시간 고래 거래 <span className="text-neutral-700">($K · 5분 단위)</span>
        </p>
            ...
            <thead className="sticky top-0 z-10 bg-neutral-900/95 backdrop-blur-sm">
              <tr>
                {["시간", "가격", "수량", "거래액", "유형"].map((h, i) => (
                  <th
                    key={h}
                    className={`px-2 py-1.5 text-[8px] font-black uppercase text-neutral-600 border-b border-neutral-800 ${
                      i === 0 ? "" : "text-right"
                    } ${i === 4 ? "text-center" : ""}`}
                  >
                    {h}
            ...
            <tbody>
              {visible.map((t, i) => (
                <tr
                  ...
                  } ${t.isBuy ? "border-l-2 border-l-emerald-500/30" : "border-l-2 border-l-rose-500/30"}`}
                >
                  <td className="px-2 py-1.5 text-[9px] font-mono text-neutral-500">
                    {new Date(t.time).toLocaleTimeString("ko-KR", { hour12: false })}
                  </td>
                  <td className="px-2 py-1.5 text-[10px] font-black text-white text-right">
                    ${t.price.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </td>
                  <td className="px-2 py-1.5 text-[9px] text-neutral-400 text-right">
                    {t.qty >= 1 ? t.qty.toFixed(3) : t.qty.toFixed(4)}{" "}
                    <span className="text-neutral-700">{ticker}</span>
                  </td>
                  <td className="px-2 py-1.5 text-[10px] font-black text-right">
