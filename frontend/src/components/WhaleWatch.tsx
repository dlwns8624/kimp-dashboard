        ...
        <div className="flex bg-neutral-950 rounded-lg border border-neutral-800 overflow-hidden">
          {SYMBOLS.map(s => (
            <button
              key={s}
              onClick={() => setSymbol(s)}
              className={`px-2 py-1 text-xs md:px-2.5 md:py-1.5 md:text-xs font-bold transition-colors ${
        ...
        {(["h1","h4","h12","h24"] as const).map((k, i) => {
          ...
          return (
            <div key={k} className={`p-2 md:p-2.5 ${i < 3 ? "border-r border-neutral-800" : ""}`}>
              <p className="text-xs md:text-xs font-bold text-neutral-500 mb-1">
                {k === "h1" ? "1H" : k === "h4" ? "4H" : k === "h12" ? "12H" : "24H"}
              </p>
              <p className="text-xs md:text-xs font-bold text-emerald-400 leading-none">{fmt$(s.buy)}</p>
              <p className="text-xs md:text-xs font-bold text-rose-400 leading-none mt-0.5">{fmt$(s.sell)}</p>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {visible.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-2 py-12">
            ...
            <p className="text-sm md:text-xs text-neutral-600 font-bold">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10 bg-neutral-900/95 backdrop-blur-sm">
              <tr>
                {["시간", "가격", "수량", "거래액", "유형"].map((h, i) => (
                  <th
                    key={h}
                    className={`px-2 md:px-3 py-1.5 text-xs md:text-xs font-bold uppercase text-neutral-500 border-b border-neutral-800 ${
                      i === 0 ? "" : "text-right"
            ...
            <tbody>
              {visible.map((t, i) => (
                ...
                >
                  <td className="px-2 md:px-3 py-1.5 text-xs md:text-sm font-mono text-neutral-500 whitespace-nowrap">
                    {new Date(t.time).toLocaleTimeString("ko-KR", { hour12: false, hour: "2-digit", minute: "2-digit", second: "2-digit" })}
                  </td>
                  <td className="px-2 md:px-3 py-1.5 text-sm md:text-xs font-bold text-white text-right whitespace-nowrap">
                    ${t.price.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </td>
                  <td className="px-2 md:px-3 py-1.5 text-xs md:text-sm text-neutral-400 text-right whitespace-nowrap hidden sm:table-cell">
                    {t.qty >= 1 ? t.qty.toFixed(3) : t.qty.toFixed(4)}{" "}
                    <span className="text-neutral-600">{ticker}</span>
                  </td>
                  <td className="px-2 md:px-3 py-1.5 text-sm md:text-xs font-bold text-right whitespace-nowrap">
                  ...
                  <td className="px-2 md:px-3 py-1.5 text-center">
                    <span
                      className={`inline-block px-1.5 py-0.5 rounded text-xs md:text-xs font-bold whitespace-nowrap ${
                        t.isBuy
